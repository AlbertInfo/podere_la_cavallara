...
<div class="mobile-only-admin">
    <section class="admin-auth-mobile">
        <div class="admin-auth-mobile__brand">
            <img src="<?= e(admin_url('assets/img/logo.svg')) ?>" alt="Podere La Cavallara">
            <div class="admin-auth-mobile__brand-copy">
                <strong>Area amministrazione</strong>
                <span>Podere La Cavallara</span>
            </div>
        </div>

        <div class="admin-auth-mobile__card">
            <h1>Accedi</h1>
            <p>Inserisci email e password per gestire richieste, clienti e prenotazioni dall’area amministrativa.</p>
            <form method="post" class="form-grid">
                <input type="hidden" name="_csrf" value="<?= e(csrf_token()) ?>">
                <label>Email
                    <input type="email" name="email" placeholder="nome@dominio.it" required>
