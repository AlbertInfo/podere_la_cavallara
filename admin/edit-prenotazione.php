...
try {
    ...
    if ($id <= 0) {
        set_flash('error', 'ID prenotazione non valido.');
    ...
    error_log('edit-prenotazione.php error: ' . $e->getMessage());
    http_response_code(500);
    exit('Errore durante il caricamento della prenotazione.');
}
    ...
    <form class="booking-form" method="post" action="<?= e(admin_url('actions/update-prenotazione.php')) ?>">
        ...
        <section class="form-section">
            ...
            <div class="booking-form-grid">
                <label>
                    Periodo soggiorno *
                    <input class="js-date-range" type="text" name="stay_period" value="<?= e((string) ($prenotazione['stay_period'] ?? '')) ?>" readonly required>
                </label>
                ...
                <label>
                    ...
                    <select name="room_type" required>
                        ...
                        <?php foreach ($rooms as $room): ?>
                            <option value="<?= e($room) ?>" <?= (($prenotazione['room_type'] ?? '') === $room) ? 'selected' : '' ?>>
                                <?= e($room) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <label>
                    Adulti *
                    <input type="number" name="adults" min="0" step="1" value="<?= (int) ($prenotazione['adults'] ?? 0) ?>" required>
                </label>

                <label>
                    Bambini
                    <input type="number" name="children_count" min="0" step="1" value="<?= (int) ($prenotazione['children_count'] ?? 0) ?>">
                </label>
            </div>
        </section>
        ...
        <section class="form-section">
            ...
            <div class="booking-form-grid">
                <label>
                    Nome e cognome *
                    <input type="text" name="customer_name" value="<?= e((string) ($prenotazione['customer_name'] ?? '')) ?>" required>
                </label>

                <label>
                    Email
                    <input type="email" name="customer_email" value="<?= e((string) ($prenotazione['customer_email'] ?? '')) ?>" placeholder="Non disponibile">
                </label>

                <label>
                    Telefono
                    <input type="text" name="customer_phone" value="<?= e((string) ($prenotazione['customer_phone'] ?? '')) ?>">
                </label>
            </div>
        </section>
        ...
        <section class="form-section">
            ...
            <div class="booking-form-grid">
                <label>
                    ...
                    <select name="status" required>
                        <?php foreach ($statuses as $value => $label): ?>
                            <option value="<?= e($value) ?>" <?= (($prenotazione['status'] ?? '') === $value) ? 'selected' : '' ?>>
                                <?= e($label) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <label>
                    Origine *
                    <input type="text" name="source" value="<?= e((string) ($prenotazione['source'] ?? 'website_admin')) ?>" required>
                </label>

                <label>
                    Riferimento esterno
                    <input type="text" name="external_reference" value="<?= e((string) ($prenotazione['external_reference'] ?? '')) ?>">
                </label>

                <label class="full">
                    Note
                    <textarea name="notes"><?= e((string) ($prenotazione['notes'] ?? '')) ?></textarea>
                </label>
            </div>
        </section>
